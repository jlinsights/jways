    ...
    <div id="track" className="relative -mt-20 z-20 px-6">
      <div className="max-w-6xl mx-auto">
        <motion.div
          ...
          className={`bg-white border border-slate-100 overflow-hidden transition-all duration-500 relative ${
        ...
        >
          ...
          <div className={`flex flex-col md:flex-row items-center gap-2 transition-all duration-500 ${
             searchStatus === 'success' ? 'bg-slate-50 p-4 md:p-6 border-b border-slate-100' : ''
          }`}>
            <div className={`flex-1 w-full relative transition-all ${searchStatus === 'success' ? 'md:max-w-md' : ''}`}>
              ...
              <form onSubmit={handleSearch}>
                <input
                  ...
                  placeholder="운송장 번호를 입력하세요 (예: JW-12345678)"
                  className={`w-full pl-12 pr-10 rounded-2xl bg-slate-50 text-slate-900 placeholder:text-slate-400 outline-none focus:bg-white focus:ring-2 focus:ring-jways-blue/20 transition-all font-medium ${
                    searchStatus === 'success' ? 'h-12 text-base border border-slate-200' : 'h-14 md:h-16 text-lg'
                  }`}
                  value={trackingId}
            ...
            {searchStatus === 'success' && shipment && (
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-3">
                    ...
                    <div className="bg-slate-50 p-6 lg:p-8 border-l border-slate-100 lg:h-[500px] overflow-y-auto custom-scrollbar">
                        <div className="mb-6 pb-6 border-b border-slate-200">
                            <h3 className="text-lg font-bold text-slate-900 mb-1">Status Updates</h3>
                            <p className="text-sm text-slate-500">Tracking ID: <span className="font-mono text-slate-700">{shipment.id}</span></p>
                        </div>
                        ...
                        <div className="space-y-0 relative">
                            ...
                            {shipment.steps.map((step, idx) => (
                                ...
                                >
                                    {/* Icon */}
                                    <div className={`relative z-10 w-10 h-10 rounded-full flex items-center justify-center border-4 border-slate-50 shrink-0 transition-colors ${
                                    ...
                                    <div className="pt-1 flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <p className={`font-bold text-sm ${
                                                step.status === 'current' ? 'text-jways-blue' : 'text-slate-900'
                                            }`}>
                                                {step.label}
                                            </p>
                                            <span className="text-xs text-slate-400 whitespace-nowrap ml-2">{step.date}</span>
                                        </div>
                                        <p className="text-xs text-slate-500 mb-1">{step.location}</p>
